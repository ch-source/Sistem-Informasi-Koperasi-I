...
$no=1;
$sql="SELECT * FROM tbl_pinjaman_anggota WHERE periode='$bulan' AND tahun='$tahun'";
...
$order="SELECT * FROM tbl_pinjaman_anggota WHERE periode='$bulan' AND tahun='$tahun'";
...
$pdf->ln(1);
$pdf->SetFont('Times','i',10);
$pdf->Cell(22.5,0.7,"Rekapitulasi Pinjaman Terbesar Anggota Periode: ".$bulan." Tahun: ".$tahun,0,0,'L');
$pdf->ln(1);
$no=1;
$pdf->SetFont('Times','B',8);
$pdf->Cell(1, 0.6, 'No', 1, 0, 'C');
...
$sql_x="SELECT * FROM tbl_pinjaman_anggota WHERE periode='$bulan' AND tahun= '$tahun' AND jumlah_pinjaman > 10000000";
...
while($lihat_x=mysqli_fetch_array($tampil_x)){
    ...
    $pdf->Cell(3, 0.6, $lihat_x['tgl_pinjaman'],1, 0, 'L');
    $pdf->Cell(3, 0.6, "Rp. ".number_format($lihat_x['jumlah_pinjaman'], 2, ".", "."),1, 1, 'L');
    $no++;
}

$order="SELECT * FROM tbl_pinjaman_anggota WHERE periode='$bulan' AND tahun='$tahun' AND jumlah_pinjaman > 10000000";
$query_order=mysqli_query($connect, $order);
$data_order=array();
while(($row_order=mysqli_fetch_array($query_order)) !=null){
$data_order[]=$row_order;
...
while($hasil=mysqli_fetch_object($sd))
{
    $pdf->Cell(16, 0.6,"Total Pinjaman Anggota",1, 0, '');
    $pdf->Cell(3, 0.6, "Rp. ".number_format($hasil->jumlah_pinjaman, 2, ".", "."),1, 1, '');
