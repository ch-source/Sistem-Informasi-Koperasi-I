...
if ($jp > $data_pinjaman['jumlah_pinjaman']){
    ...
    document.location.href='dashboard.php?p=data_pinjaman_anggota&id=".$id."'</script>\n";
      ...
      $query1 = "INSERT INTO `tbl_pembayaran_pinjaman` (`id_pembayaran`, `id_pinjaman`, `id_anggota`, `tgl_pembayaran`, `jumlah_pinjaman`, `jenis_bunga`, `bunga`, `jangka_waktu`, `angsuran_k`, `sisa_angsuran`, `angsuran_pokok`, `angsuran_bunga`, `total_angsuran`, `total_tunggakan`) VALUES ('$idpembayaran', '$id', '$np', '$tp', '$jp', '$jb', '$bunga', '$jw', '$ak', '$sa', '$ap', '$ab', '$ta', '$tt')";
      ...
      if ($sql1) {
        $query_r="UPDATE tbl_pinjaman_anggota SET jumlah_pinjaman='$jumlahpinjaman' WHERE id_pinjaman='$id'";
               ...
               if ($query_a) {
                ...
                if ($queryb) {
                  $queryc="UPDATE tbl_pinjaman_anggota SET angsuran_k='$angsuran' WHERE id_pinjaman='$id'";
